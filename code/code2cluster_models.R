...
for(i in 1:length(unique(mod_data$id)) ){
  ...
  data_reg_ls[[i]] <- cbind(ID = unique(mod_data$id)[i], date = ti, iLoc, 
...
if(paral_prep == T){
  ...
  data_reg_ls[[1]] <- data_reg_ls[[1]][1:10,]
  data_reg_ls[[2]] <- data_reg_ls[[2]][1:10,]
  
  prep_ss_ls <- 
  mcmapply(function(X) prepData(X, type = "LL", coordNames = c("lon", "lat"),
  ...
  ), X = data_reg_ls, mc.cores = 2)
  #mclapply(X = data_reg_ls, function(X) prepData(X, type = "LL", coordNames = c("lon", "lat"),
  #                                               covNames = c("sex", "collared"),
  #                                               spatialCovs = list(cover = ld_cov_crop_rp_reclf, 
  #                                                                  fence = fence_raster, 
  #                                                                  road = road_trunk,
  ...
  data_prep_cv2 <- prepData(data2prep[1:20,], type = "LL", coordNames = c("lon", "lat"),
                            covNames = c("sex", "collared"),
                            spatialCovs = list(cover = ld_cov_crop_rp_reclf, 
                                               fence = fence_raster, 
                                               road = road_trunk,
...
if(sum(data_prep_cv2$step == 0, na.rm = T)!=0) stop("ERROR !!!")
...
# detect multiple modes in continuous data ####
  ...
  acf(data_prep_cv$step[!is.na(data_prep_cv$step)], lag.max = 300)
  dev.off()
  png(filename = "output/acf_angle_.png")
  acf(data_prep_cv$step[!is.na(data_prep_cv$angle)], lag.max = 300)
  dev.off()
...
if(run_type[2] == "serial"){
  ...
  mclapply(X = c(5:n_models), 
           function(X) fit_HMM_mods(X, data_prep_cv, "output/", retryFits = 10), mc.cores = n_models)
}
